...
variable "pve_ssh_key_public" {
  description = "File path to public SSH key for PVE - used for cloud-init"
  type        = string
  sensitive   = true
  default     = null
}
...
variable "pve_bios" {
  ...
  default     = "seabios"
  validation {
    condition     = contains(["seabios", "ovmf"], var.pve_bios)
    error_message = "Invalid bios setting: ${var.pve_bios}. Valid options: 'seabios' or 'ovmf'."
  }
}
